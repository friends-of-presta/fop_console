...
if ('/path/to/prestashop/' === getenv('_PS_ROOT_DIR_')) {
    throw new Exception(PHP_EOL . PHP_EOL . ">>> You MUST configure the path to Prestashop in \"$ps_root_env_file_path\" or check _PS_ROOT_DIR_" . PHP_EOL . PHP_EOL);
}

// abort if _PS_ROOT_DIR_ is not a valid path
if (!file_exists(getenv('_PS_ROOT_DIR_'))) {
    throw new Exception(PHP_EOL . PHP_EOL . ">>> invalid path to prestashop. Provided : ".getenv('_PS_ROOT_DIR_')." Edit \"$ps_root_env_file_path\" or check _PS_ROOT_DIR_ ");
}
